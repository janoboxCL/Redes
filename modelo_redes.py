...
def _tipo_max_por_nodo(
    ...
    *,
    ...
    for rut, g in df.groupby("RUT_RELACIONADO"):
        if g.empty:
            pesos[str(rut)] = 1.0; tipos[str(rut)] = "OTRO"; continue
        maxw = float(g["__PESO__"].max())
        tipos_max = sorted(set(g.loc[g["__PESO__"] == maxw, "__TIPO_NORM__"].astype(str)))
        pesos[str(rut)] = maxw if np.isfinite(maxw) else 1.0
        tipos[str(rut)] = " | ".join(tipos_max) if tipos_max else "OTRO"
    return pesos, tipos
...
def calcular_riesgo_red_auto(
    ...
    ***IMPORTANTE***: La parte gráfica no se toca.
    ...
    peso_por_nodo, tipo_sel_por_nodo = _tipo_max_por_nodo(
    ...
    for rut, row in marcas_df.iterrows():
        ...
        tipo_txt = tipo_sel_por_nodo.get(rut, "OTRO")
